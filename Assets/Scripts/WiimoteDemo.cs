...
public class WiimoteDemo : MonoBehaviour {
    ...
    public Rigidbody PlayerToMove;

    public bool StrumIsDown, StrumIsUp;

    public float StrumDownTime, StrumUpTime;
	...
	void Update () {
        ...
        */
        ...
        if (StartStrumDownTimer == true)
        {
            StrumDownTime -= Time.deltaTime;
        } 

        if (StrumDownTime >= 0 && StartStrumDownTimer)
        {
            StrumIsDown = true;
        }
        else if (StrumDownTime <= 0 || StartStrumDownTimer == false)
        {
            StrumIsDown = false;
        }

        if (StartStrumUpTimer == true)
        {
            StrumUpTime -= Time.deltaTime;
        }

        if (StrumUpTime >= 0 && StartStrumUpTimer)
        {
            StrumIsUp = true;
        }
        else if (StrumUpTime <= 0 || StartStrumUpTimer == false)
        {
            StrumIsUp = false;
        }
